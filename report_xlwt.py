...
def process_sample(job, config, sample, samples, addresses, authenticator, thresholds, callers):
    ...
    pass_style = xlwt.easyxf('pattern: pattern solid, fore_colour bright_green;')
    ...
    for sheet in tier_sheets:
        ...
        for variant in report_data['variants'][tier_key[sheet_num]]:
            ...
            sheet.write(row, 9, "{}".format(",".join(variant.callers) or None))
            sheet.write(row, 10, "{}".format(",".join(variant.cosmic_ids) or None))
            sheet.write(row, 11, "{}".format(variant.cosmic_data['num_samples']))
            sheet.write(row, 12, "{}".format(variant.cosmic_data['aa']))
            ...
            sheet.write(row, 21, "{}".format(variant.min_depth))
            sheet.write(row, 22, "{}".format(variant.max_depth))
            sheet.write(row, 23, "{}".format(variant.chr))
            sheet.write(row, 24, "{}".format(variant.pos))
            sheet.write(row, 25, "{}".format(variant.end))
            sheet.write(row, 26, "{}".format(",".join(variant.rs_ids)))

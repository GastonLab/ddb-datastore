...
def process_sample(job, config, sample, samples, addresses, authenticator, thresholds, callers):
    ...
    for sheet in tier_sheets:
        ...
        for variant in report_data['variants'][tier_key[sheet_num]]:
            ...
            sheet.write(0, 0, "{}".format(variant.sample))
            sheet.write(0, 1, "{}".format(variant.library_name))
            sheet.write(0, 2, "{}".format(variant.gene))
            sheet.write(0, 3, "{}".format(variant.amplicon_data['amplicon']))
            sheet.write(0, 4, "{}".format(variant.ref))
            sheet.write(0, 5, "{}".format(variant.alt))
            sheet.write(0, 6, "{}".format(variant.codon_change))
            sheet.write(0, 7, "{}".format(variant.aa_change))
            sheet.write(0, 8, "{}".format(variant.max_som_aaf))
            sheet.write(0, 9, "{}".format(variant.num_times_called))
            sheet.write(0, 10, "{}".format(variant.vaf_median))
            sheet.write(0, 11, "{}".format(variant.vaf_std_dev))
            sheet.write(0, 12, "{}".format(",".join(variant.callers) or None))
            sheet.write(0, 13, "{}".format(variant.num_times_callers))
            sheet.write(0, 14, "{}".format(",".join(variant.cosmic_ids) or None))
            sheet.write(0, 15, "{}".format(variant.cosmic_data['num_samples']))
            sheet.write(0, 16, "{}".format(variant.cosmic_data['aa']))
            sheet.write(0, 17, "{}".format(variant.clinvar_data['significance']))
            sheet.write(0, 18, "{}".format(variant.clinvar_data['hgvs']))
            sheet.write(0, 19, "{}".format(variant.clinvar_data['disease']))
            sheet.write(0, 20, "{}".format(coverage_string))
            sheet.write(0, 21, "{}".format(reads_string))
            sheet.write(0, 22, "{}".format(variant.impact))
            sheet.write(0, 23, "{}".format(variant.severity))
            sheet.write(0, 24, "{}".format(variant.max_maf_all))
            sheet.write(0, 25, "{}".format(variant.min_depth))
            sheet.write(0, 26, "{}".format(variant.max_depth))
            sheet.write(0, 27, "{}".format(variant.chr))
            sheet.write(0, 28, "{}".format(variant.start))
            sheet.write(0, 29, "{}".format(variant.end))
            sheet.write(0, 30, "{}".format(",".join(variant.rs_ids)))

        ...
        for variant in ordered:
            ...
            if variant.amplicon_data['amplicon'] is 'None':
                ...
                if assignable:
                    ...
                    if variant.cosmic_ids or any(
                        i in tier1_clinvar_terms for i in variant.clinvar_data[
                            'significance']):
                        tier = 1
                    elif variant.severity == 'MED' or variant.severity == 'HIGH':
                        tier = 3
                    else:
                        tier = 4

                    if variant.max_som_aaf < thresholds['min_saf'] or variant.max_depth < thresholds['depth']:
                        filter_pass = False

                    if (tier == 1 or tier == 3) and filter_pass == True:
                        ...
                        variant.vaf_median = "NA"
                        variant.vaf_std_dev = "NA"
                        variant.run_median = "NA"
                        variant.vaf_perc_rank = "NA"
                        variant.num_times_called = "NA"
                        variant.num_times_run = "NA"
                        variant.matching_samples = "NA"
                        variant.num_times_callers = "NA"

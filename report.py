...
def get_amplicon_coverage(targets):
    amplicon_coverage = defaultdict(lambda: defaultdict(float))

    for amp in targets:
        coverage_data = SampleCoverage.objects.timeout(None).filter(
            SampleCoverage.sample == samples[sample]['sample_name'],
            SampleCoverage.amplicon == amp,
        ...
        for result in coverage_data:
            ...
            amplicon_coverage[amp]['num_reads'] = result.num_reads
            amplicon_coverage[amp]['mean_coverage'] = result.mean_coverage

    return amplicon_coverage


def prep_coverage_report(sample, report_root):
    with open("{}_coverage_{}.txt".format(sample, report_root), "w") as coverage_report:
...
def prep_log(sample, report_root):
    with open("{}.{}.log".format(sample, report_root), 'w') as logfile:
